...
1. Create `sol_max_planner` with `model: gpt-5.6-sol`, `reasoning_effort: max`, and no inherited task context. Before substantive inspection, instruct it to derive and record `SCOPE_LOCK` from the user request and approved user decisions; if material ambiguity would change the concern or outcome, return `USER_DECISION_REQUIRED`. Instruct it to inspect read-only only what directly resolves `SCOPE_LOCK` and return the exact `## PLAN` protocol artifact. Do not let it modify product files. Plan detail has no maximum, no minimum, and no size target; include only detail directly required to implement and validate `SCOPE_LOCK`. Completeness means resolving only the in-lock happy, failure, and boundary behavior; direct dependencies/callers; tests; and validation. Every implementation step, change, and test must map to an acceptance criterion or preservation invariant. Record adjacent discoveries only as `OUT_OF_SCOPE_FINDINGS` with `NO ACTION`; a new concern, outcome, or broader scope requires `USER_DECISION_REQUIRED`. After requesting the plan, continue waiting for the planner until it returns a complete `## PLAN`; do not time out, proceed, or start Terra based on an incomplete, delayed, or absent plan. Save the planner's exact, complete `## PLAN` artifact as a Markdown file in the target repository's `./.plan/` directory. Do not summarize, paraphrase, trim, merge, or otherwise rewrite it before persistence or handoff.
...
- Sol planning, decisions, replans, final review, and Terra execution may narrow, reorder, or deepen work only within `SCOPE_LOCK`; none may broaden it. Terra edits, tests, and verifies only the current approved plan or exact Sol decision. It must fail closed when a meaningful choice, mismatch, or unplanned scope appears; a broader concern, outcome, or optional adjacent task requires `USER_DECISION_REQUIRED`.
...
- Ask Sol to replan the remaining work after two decision requests in one implementation step or when the protocol's replan signals occur. Do not give Terra a stale plan after that request. Replanning may narrow, reorder, or deepen the current `SCOPE_LOCK`; broadening it requires `USER_DECISION_REQUIRED`.
