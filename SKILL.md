...
1. Create `sol_max_planner` with `model: gpt-5.6-sol`, `reasoning_effort: max`, and no inherited task context. Instruct it to inspect the repository read-only and return the exact `## PLAN` protocol artifact. Do not let it modify product files. Impose no workflow length, line, word, or token ceiling and no concision target on the plan. Never aim for, stop at, or compress the artifact to approximately 100 lines. Continue investigation and writing until every repository-grounded implementation decision is resolved and Terra Low can perform the work with only the explicitly allowed mechanical discretion. If Terra would still need to choose architecture, control flow, data or error semantics, caller effects, edge-case behavior, test design, or validation criteria, the plan is incomplete. After requesting the plan, continue waiting for the planner until it returns a complete `## PLAN`; do not time out, proceed, or start Terra based on an incomplete, delayed, or absent plan. Save the planner's exact, complete `## PLAN` artifact as a Markdown file in the target repository's `./.plan/` directory. Do not summarize, paraphrase, trim, merge, or otherwise rewrite it before persistence or handoff.
