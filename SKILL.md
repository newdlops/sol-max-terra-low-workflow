...
1. Create `sol_max_planner` with `model: gpt-5.6-sol`, `reasoning_effort: max`, and no inherited task context. Instruct it to inspect the repository read-only and return the exact `## PLAN` protocol artifact. Do not let it modify product files.
...
- Terra edits, tests, and verifies only the current approved plan or exact Sol decision. It must fail closed when a meaningful choice, mismatch, or unplanned scope appears.
...
- Treat each PLAN/DECISION artifact as immutable until superseded by a Sol decision. Label amendments with a new plan version and pass the full current plan to Terra.
